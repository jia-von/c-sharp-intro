    ...
    class Program
    {
        ...
        static int CollectIntFromUser()
        {
            ...
            while (error==true)
            {
                 ...
                 try //Wrap potentially-failing code in a try - this will prevent an unhandled excpetion (fatal error for your program.)
                 {
                     intValue = int.Parse(userValue); // Attempt to convert the string...
                     error = false; //If we get here, we're good to return the int!
                 }
                 catch (Exception exception)
                 {// We use "catch" to decide what happens if the "Try"has a value
